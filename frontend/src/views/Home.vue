...
function quickTest() {
  rawContent.value = '# PPT 演讲助手 — AI 驱动的演示文稿生成与双屏演讲工具\n\n## 产品概述\nPPT 演讲助手是一款全栈 AI 演讲工具，支持 Web 浏览器和 Windows / macOS / Linux 桌面应用。只需输入文档内容或粘贴 Wiki URL，AI 即可自动分析结构、拆分页面、生成 PPT 和配套演讲旁白，实现演示屏 + 旁白屏双屏同步演讲。\n\n## 核心功能\n- **多源内容输入**：支持手动粘贴 Markdown/HTML、Docmost Wiki 导入、本地上传文件\n- **多模型 AI 生成**：LLM Gateway / Claude / OpenAI / Ollama，灵活切换\n- **智能页面拆分**：AI 自动分析文档结构，生成封面、目录、内容、数据、结语等多种布局\n- **演讲旁白生成**：每页生成配套演讲稿、关键词提示、演讲技巧\n- **富文本编辑器**：可视化调整标题、内容、时长、旁白，拖拽排序页面\n- **双屏演示模式**：演示屏全屏播放 PPT，旁白屏同步显示演讲稿 + 倒计时 + 关键词\n- **智能计时系统**：每页剩余时间、演讲已过时长、进度条提醒，支持暂停/重置\n- **本地数据持久化**：SQLite 数据库存储所有项目，离线可用\n- **跨平台桌面应用**：Electron 42 打包，原生多窗口 + BroadcastChannel 同步\n- **导出功能**：支持导出 HTML 和 PDF 格式\n\n## 使用流程\n1. **输入内容** — 粘贴文档、上传文件、或输入 Wiki URL\n2. **选择风格** — 商务 / 科技 / 简约 / 教育四种 PPT 模板\n3. **选择模型** — LLM Gateway / Claude / OpenAI / Ollama\n4. **一键生成** — AI 自动分析、拆分、生成 PPT 和旁白\n5. **编辑微调** — 修改标题、内容、时长、旁白文本\n6. **开始演示** — 演示屏 + 旁白屏自动同步翻页和计时\n\n## 风格模板\n- **商务风格**：简洁专业，深蓝主调，适合企业汇报和客户提案\n- **科技风格**：现代动感，深色背景+青色高亮，适合技术分享和产品发布\n- **简约风格**：清爽优雅，浅色调，适合学术报告和培训课程\n- **教育风格**：高对比度，绿色主调，适合教学课件\n\n## 技术架构\n- **前端框架**：Vue 3 + Vite 8 + Pinia 状态管理\n- **UI 样式**：Tailwind CSS 4\n- **PPT 引擎**：Reveal.js 6\n- **本地数据库**：SQLite（sql.js WebAssembly）\n- **AI 集成**：LLM Gateway / Claude API / OpenAI API / Ollama 本地模型\n- **同步机制**：BroadcastChannel（浏览器 / Electron 桌面通用）\n- **桌面打包**：Electron 42 + electron-builder\n- **Markdown 解析**：marked\n\n## 双屏操作指南\n- **翻页**：键盘 ← → 或点击按钮，演示屏和旁白屏自动同步\n- **暂停/继续**：Space 键或点击暂停按钮\n- **重置计时**：点击 ↺ 重置当前页倒计时\n- **页面跳转**：旁白屏上方页签点击任意页快速跳转\n- **返回编辑**：旁白屏左上角 ← 按钮返回编辑器\n\n## 桌面应用\n支持一键打包为 Windows (.exe)、macOS (.dmg)、Linux (.AppImage) 三大平台原生应用。打包后获得原生多窗口体验：编辑器、演示屏、旁白屏各自独立窗口，跨屏同步基于 Chromium BroadcastChannel。\n\n## 总结\nPPT 演讲助手将 AI 内容生成、PPT 设计、双屏演讲、跨平台桌面应用融为一体，大幅提升演讲准备效率，让每一位演讲者都能专注于内容表达。'
